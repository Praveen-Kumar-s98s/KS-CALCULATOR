...
const SilverCalculator = ({ onGoToGold, onGoToInterest }) => {
  // State for silver rate
  ...
  // Load silver rate from localStorage on component mount
  useEffect(() => {
    const savedSilverRate = localStorage.getItem('jewellerySilverRate');
    if (savedSilverRate) {
      setCurrentSilverRate(parseFloat(savedSilverRate));
    }
  }, []);
  ...
  // Set silver rate
  const handleSetSilverRate = () => {
    const rate = parseFloat(silverRate);
    if (!isNaN(rate) && rate > 0) {
      setCurrentSilverRate(rate);
      localStorage.setItem('jewellerySilverRate', rate.toString());
      setSilverRate('');
    }
  };
  ...
  const handleCalculate = () => {
    if (!currentSilverRate || !weight || !wastage || !makingCostValue) {
    ...
    // Calculations
    const silverValue = currentSilverRate * weightValue;
    const wastageAmount = silverValue * (wastageValue / 100);
    const subtotal = silverValue + wastageAmount;
  ...
  const handlePrintBill = () => {
    ...
    const printContent = `
      <html>
        <head>
          <title>Silver Bill - ${itemName || 'Item'}</title>
          <style>
        ...
        <body>
          <div class="header">
            <div class="shop-name">🥈 KS Jewellery - Silver</div>
            <div>Silver Price Calculation Bill</div>
          ...
          <div class="item-details">
            ...
            <p><strong>Silver Rate:</strong> ₹${currentSilverRate.toLocaleString('en-IN')}/gram</p>
          ...
          <table class="calculation-table">
            ...
            <tbody>
              <tr>
                <td>Silver Value (${currentSilverRate.toLocaleString('en-IN')} × ${calculations.weight})</td>
                <td>${calculations.silverValue.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
              </tr>
              <tr>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-4 px-2 sm:py-8 sm:px-4">
      <div className="max-w-4xl mx-auto">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Enter Today's Silver Rate (₹/gram)
                  </label>
                ...
                {currentSilverRate > 0 && (
                  <div className="bg-gray-50 border border-gray-200 rounded-md p-3">
                    <p className="text-sm text-gray-600">Current Silver Rate:</p>
                    <p className="text-base sm:text-lg font-semibold text-gray-700">
                      {formatCurrency(currentSilverRate)}/gram
                    </p>
                  </div>
                )}
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      placeholder={makingCostType === 'percentage' ? 'e.g., 10' : 'e.g., 500'}
                      className="currency-input"
                    />
                  </div>
                </div>
          ...
          <div className="space-y-4 sm:space-y-6">
              ...
              <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
                ...
                <div className="space-y-2 sm:space-y-3">
                  <div className="flex justify-between py-2 border-b border-gray-200">
                    <span className="text-gray-600 text-sm sm:text-base">Silver Value:</span>
                    <span className="font-medium text-sm sm:text-base">{formatCurrency(calculations.silverValue)}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-gray-200">
                  ...
                  <div className="flex justify-between py-3 bg-gray-50 rounded-md px-3 sm:px-4 border-2 border-gray-300">
                    <span className="text-gray-800 font-bold text-base sm:text-lg">Final Price:</span>
                    <span className="font-bold text-gray-800 text-lg sm:text-xl">{formatCurrency(calculations.finalPrice)}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4">
              ...
              <ul className="text-xs sm:text-sm text-blue-700 space-y-1">
                <li>• Set today's silver rate first</li>
                <li>• Fill in all required fields (marked with *)</li>
                <li>• Choose making cost as percentage or fixed amount</li>
                <li>• Silver rate is saved locally for your convenience</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="text-center mt-6 sm:mt-8 py-3 sm:py-4 border-t border-gray-200">
          <p className="text-xs sm:text-sm text-gray-600">
            Built by <span className="font-semibold text-gray-700">Praveen Kumar (SPK)</span>
          </p>
        </div>
      </div>
    </div>
